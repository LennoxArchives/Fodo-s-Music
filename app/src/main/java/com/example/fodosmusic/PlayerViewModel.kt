...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Lagu yang di-like, disimpan sebagai kumpulan id. In-memory aja (reset kalau app di-kill),
    // gampang diganti nanti ke DataStore/Room kalau mau persist.
    var likedSongs = mutableStateOf<Set<Long>>(emptySet())
        private set
    ...
    // Dipakai buat nampilin queue (previous/next) di Playlist sheet.
    ...
    fun toggleLike(songId: Long) {
        likedSongs.value = if (likedSongs.value.contains(songId)) {
    ...
    fun isLiked(songId: Long): Boolean = likedSongs.value.contains(songId)
